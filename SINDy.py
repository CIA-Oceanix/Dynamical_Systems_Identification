...
class SINDy:
    def __init__(self,data,polyorder=2,usesine=False,cutoff=0.025,sparsity_itterations = 10):
        ...
        self._cutoff    = cutoff
        self._sparsity_itterations    = sparsity_itterations
        self._theta     = []
        self._xi        = []
        self._rx        = []
    ...
    def PolyConvolve(self, data1, data2, initialrun=False):
        ...
        for n in range(np.shape(data1)[0]):
            a, b = data1[n], data2[n]
            for i in range(len(a)):
                for j in range(min(i,len(b)-1),len(b)):
                    row.append(a[i]*b[j])
            res.append(row)
            row = []
    ...
    def SparsifyDynamics(self):
        ...
        self._xi = np.linalg.lstsq(self._theta,self._dx,rcond=-1)[0]
        print ("Iteration in progress: ",)
        for k in range(self._sparsity_itterations):
            print ("{},".format(k+1),)
            ...
            for ind in range(self._dim):
                ...
                self._xi[biginds,ind] = np.linalg.lstsq(self._theta[:,biginds],self._dx[:,ind],rcond=-1)[0]
        print ("")
